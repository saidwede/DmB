...
const img = ref('')
const backendURL = ref(import.meta.env.VITE_APP_API_URL)
...
function saveProfilePic() {
  if (profilCanva.value != null) {
    ...
    axios
      .post(`upload-profile-pic/${userState.user.id}`, formData, {
        headers: {
...
<template>
  <div class="card-contaner">
    ...
    <div class="profile-container">
      ...
      <img
        :src="`${profilePic != '/img/person.png' ? backendURL + '/' : ''}${profilePic}`"
        alt=""
        srcset=""
        class="profile-pic"
      />
  ...
  <div :class="`simple-modal ${showProfileModal ? 'is-active' : ''}`">
    <div class="modal-content" ref="modalContentRef">
      <cropper
        ...
        class="cropper"
      />
      <button @click="saveProfilePic" class="std-btn rd-btn btn-green">Sauvegarder</button>
    </div>
  </div>
</template>
